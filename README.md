...
[![Skills Suite](https://img.shields.io/badge/Skills%20Suite-400%2B%20Skills-brightgreen?style=for-the-badge)](https://github.com/omspradippatil/Claude-Skills)
[![Platforms](https://img.shields.io/badge/Platforms-macOS%20%7C%20Linux%20%7C%20Windows-blue?style=for-the-badge)](https://github.com/omspradippatil/Claude-Skills)
[![Agents](https://img.shields.io/badge/Agents-Claude%20%7C%20Antigravity%20%7C%20Cursor%20%7C%20Codex-purple?style=for-the-badge)](https://github.com/omspradippatil/Claude-Skills)

**The ultimate, non-conflicting, multi-platform skills installer for AI coding assistants.**

*Enrich your agent with taste-enforced UI/UX, verifiable architecture visualization, mobile & web engineering heuristics, agentic SEO, backend architectures, workflow automation, and persistent cross-session memory.*

</div>

---

## 🚀 One-Liner Quick Install (Universal Terminal Execution)
...
When launched, the installer offers an intuitive 2-step setup:
...
Where would you like to install the skills?
  ...
  2) 📁 Current Project   (Install only in this project folder: ./.claude/skills & ./.agent/skills)
```
...
> **Zero Prompt Automation**: Once you make your selection, all sub-skills are **automatically selected and confirmed by default** (`--all -y`). You never have to manually select individual packages!

---

## ⚡ Direct Non-Interactive CLI Flags

You can also pass CLI flags directly to skip all prompts:

| Flag | Purpose | Example |
| :--- | :--- | :--- |
| `--global` / `-g` | Install system-wide into `~/.agents/skills` and `~/.claude/skills` | `./install.sh --global --all` |
| `--project` / `-p` | Install into the current project directory (`./.claude/skills`) | `./install.sh --project --all` |
| `--mobile` / `-m` | Install only Mobile Application skills | `./install.sh --project --mobile` |
| `--web` / `-w` | Install only Web Application skills | `./install.sh --project --web` |
| `--uninstall` / `-u` | Completely remove all installed skills and CLIs | `./install.sh --uninstall` |

---

## 🧠 What "Loading" Means in Skill (`SKILL.md`) Files

In modern AI agent systems (such as Claude Code, Antigravity, Cursor, Codex, and OpenCode), skills do **not** flood the context window on startup. Instead, they use a **Progressive Disclosure & Lifecycle Loading System**:
...
└──────────────────────────────┬───────────────────────────────┘
                               │ User sends prompt matching domain
                               ▼
┌──────────────────────────────────────────────────────────────┐
│ 2. Progressive Disclosure (On-Demand Loading)                │
│    Agent injects full `SKILL.md` workflows, rules, and       │
│    decision trees into active context ONLY when triggered.   │
└──────────────────────────────┬───────────────────────────────┘
                               │ Agent plans and collects evidence
                               ▼
┌──────────────────────────────────────────────────────────────┐
...
1. **Lightweight Indexing**: Only skill headers are indexed at startup (~20 tokens). Context stays clean and ultra-fast.
2. **Context Activation**: The agent only reads full instructions when your prompt touches that domain (e.g. *"Generate a design token system"* or *"Audit our schema for Google SEO"*).
3. **Evidence Collectors**: Skills with helper scripts (like `Archify`, `Playwright`, `Agentic SEO`, `UI Pro Max`) execute locally without third-party API dependencies.
4. **Zero-Conflict Boundaries**: Skills are partitioned into isolated, complementary domains. UI rules never clash with database migrations or SEO indexing.

---

## 📦 Complete Skills Suite Breakdown & Activation Prompts
...
| Skill / Tool | Repository / Package | Purpose & Loading Behavior | Example Activation Prompt |
| :--- | :--- | :--- | :--- |
...
| **Web Design Collection** | `MengTo/Skills` | Specialized CSS keyframes, layout transitions, scroll-driven effects, and Framer Motion patterns for modern websites (130+ skills). | `"Create a smooth scroll-driven card expansion effect using Framer Motion and Tailwind CSS."` |
...
- **Initialize Design Tokens**: Inside any UI project folder, run:
  ```bash
  uipro init --ai antigravity
  ```
- **Local Execution**: You can also clone this repository and run the scripts locally:
  - macOS/Linux: `./install.sh` (or `./install.sh --project`)
  - Windows: `.\install.ps1` (or `.\install.ps1 -Project`)
